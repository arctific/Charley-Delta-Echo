...
#20160616 DWT 0.3 Handle Command Error Codes
#
...
if($run_state > 1) {
  ...
  $health_state++ ;
	...
	printf("# Antivirus possibily running.\n") ;
} else {
  printf("### Antivirus not running...\n") ;
}
#
...
while(<CMD>) {
  ...
  if(/ProductVersion\s+REG_SZ\s+([0-9\.]+)(\s*$)/) {
    ...
    printf("### Antivirus version not identified.\n") ;
  }
}
close(CMD) ;
#
...
while(<CMD>) {
  ...
  if(/PatternFileDate\s+REG_BINARY\s+([0-9A-Fa-f]+)(\s*$)/) {
    ...
    printf("### Antivirus signature updates not identified.\n") ;
  }
}
close(CMD) ;
exit 0 ;
